<script>
    $(document).ready(function () {
        let table = $('#arsip_tb').DataTable({
            "ajax": {
                "url": "<?= base_url('arsip/getSuratData/' . $selected_surat_id); ?>", // Load data based on the selected surat ID
                "type": "GET",
                "dataSrc": ""
            },
            "columns": [
                ...
                {"data": "file_name"},
                {
                    ...
                    "render": function (data, type, row) {
                        // Customize the action buttons as needed
                        return `
                    <button class="btn btn-info btn-sm view-btn" data-id="${row.id}" data-nama="${row.nama_surat}" data-nomor="${row.nomor_surat}">View</button>
        ...
        $('#arsip_tb tbody').on('click', '.view-btn', function () {
            let suratId = $(this).data('id');
            let namaSurat = $(this).data('nama');
            let nomorSurat = $(this).data('nomor');

            // Fetch the details using AJAX
            $.get(`<?= base_url('arsip/getSuratData/') ?>${suratId}`, function (data) {
                console.log(data);
                // Populate the modal with the received data
                $('#detailModal .modal-body').html(`
            <h5>Nama Surat: ${namaSurat}</h5>
            <h5>Nomor Surat: ${nomorSurat}</h5>

            <embed src="<?= base_url('arsip/viewPDF/') ?>${suratId}" width="100%" height="400px" />
        `);
